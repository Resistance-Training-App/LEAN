...
class SaveMetricsTest: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        exercise_2.isLearnt = true
        exercise_2.isRotation = false
    ...
    func testSaveMetrics() throws {
        saveMetrics(workout: workoutManager,
                    motion: motionManager,
                    viewContext: coreDataStack.testContext)
        
        XCTAssertNotNil(motionManager.metrics[0][0])
        XCTAssertTrue(motionManager.metrics[0][0].repCount == 0)
        XCTAssertTrue(motionManager.metrics[0][0].order == 0)
        XCTAssertTrue(motionManager.metrics[0][0].results == [])
        XCTAssertTrue(motionManager.metrics[0][0].repStartTimes == [])
        XCTAssertTrue(motionManager.metrics[0][0].repEndTimes == [])
        XCTAssertTrue(motionManager.metrics[0][0].repRangeOfMotions == [])
    }
}
