...
class StatisticsTests: XCTestCase {
    ...
    func testAddWorkoutHistories() throws {
        ...
        let fetchRequest = Statistics.fetchRequest()
        let statistics = try coreDataStack.testContext.fetch(fetchRequest)

        XCTAssertTrue(profile.statistics?.workouts == 1)
        XCTAssertTrue(profile.statistics?.time == workoutManager.timer.secondsElapsed)
        
        profile.statistics = statistics.first

        addWorkoutHistory(workout: workoutManager,
                          profile: profile,
                          viewContext: coreDataStack.testContext)
        
        XCTAssertTrue(profile.statistics?.workouts == 2)
        XCTAssertTrue(profile.statistics?.time == workoutManager.timer.secondsElapsed * 2)
    }
    
    func testRemoveWorkoutHistories() throws {
        
        addWorkoutHistory(workout: workoutManager,
                          profile: profile,
                          viewContext: coreDataStack.testContext)
        
        let statistics = try coreDataStack.testContext.fetch(Statistics.fetchRequest())
        
        profile.statistics = statistics.first
        
        let workoutHistories = try coreDataStack.testContext.fetch(WorkoutHistory.fetchRequest())

        removeWorkoutHistory(workoutHistory: workoutHistories.first!,
                             profile: profile,
                             viewContext: coreDataStack.testContext)
        
        XCTAssertTrue(profile.statistics?.workouts == 0)
