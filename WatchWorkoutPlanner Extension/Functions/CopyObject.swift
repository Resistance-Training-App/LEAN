...
extension NSManagedObject {
    ...
    func cloneObject(context: NSManagedObjectContext,
                     cache alreadyCopied: inout Dictionary<NSManagedObjectID, NSManagedObject>) -> NSManagedObject {
                ...
                if value.isToMany {
                    if let sourceSet = self.value(forKey: key) as? NSMutableOrderedSet {
                        ...
                        while let relatedObject = nextObject {
                            let clonedRelatedObject = relatedObject.cloneObject(context: context,
                                                                            cache: &alreadyCopied)
